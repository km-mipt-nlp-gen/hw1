...
5. Инструкция по запуску ([ссылка](#5-инструкция-по-запуску))
6. Описание использования моделей (классы, методы, ресурсы реализующие логику выдачи ответов)([ссылка](#6-описание-использования-моделей))
7. Ускорение вывода, а также итеративный процесс улучшений проекта (опыт, выводы, подходы и методы)([ссылка](#7-ускорение-и-итеративные-улучшения))
...
- **web_app/src** - модули реализующие трехуровневую архитектуру с вспомогательной логикой:
  ...
  - *chat_service_module.py* (репозиторий, кеширующий эмбеддинги, полученные после инференса Siamese-Be-Encoder, описанные в пункте 2, для расширяемости);
...
- в самих ноутбуках (в дополнение к препроцессингу данных и коду для сборки, запуска обучения, тестов и приложения) есть все упомянутые в отчете графики, таблицы, логи тестов и старта приложения и тестами;
...
- для ускорения процессинга используется модуль chat_service_accelerator_module (для ускорения требуются неизменные эмбеддинги контекстов и ответов целевого персонажа, а также эмбеддинги ответов целевого персонажа для скрещивания с кастомным запросом от пользователя);
- более того, в chat_service_accelerator_module создаются 2 FAISS индекса для рассчета L2 меры вместо косинусного расстояния, в том числе с использованием PSA;
...
- всего пользователь может использовать 3 из 4 ресурса для запросов к серверу, где используется Bi-Encoder, для обоих режимов диалога:
  - **top_cos_sim_bi_cr** (Bi-Encoder через cosine similarity, далее Cross-Encoder для реранкинга);
  - **top_l2_bi_cr** (Bi-Encoder через L2, далее Cross-Encoder для реранкинга);
  - **top_l2_psa_bi_cr** (Bi-Encoder через L2 с PSA для уменьшения размерности, далее Cross-Encoder для реранкинга);
...
- Cross-Encoder требовал много памяти, была создана обработка по батчам для **top_cr** в find_top_n_unique_answers_cross_enc;
- Siamese-Bi-Encoder отрабатывал медленно, поэтому был создан класс ускоретель и кешированы эмбеддинги (упомянуты в пунктах 6.1, 3.1, 2);
